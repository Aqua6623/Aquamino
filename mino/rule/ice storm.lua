...
function rule.init(P,mino)
    mus.add('music/Hurt Record/Risky Area','whole','mp3',95.743,157.377)
    mus.start()
    --mino.player[1].w=4 mino.player[1].h=4
...
function rule.rise(player,col)
    ...
    if ice.H<0 then ice.H=0
        ice.speed=(.02+player.stormLv*.015)*(.75+.5*rand())
    end
end
...
function rule.lvup(player,mino)
    ...
    while player.iceScore>=rule.scoreUp*(player.stormLv-1)+rule.scoreBase do
        ...
        if 12==player.stormLv then mino.win() return end
